...
# File paths based on installation method
...
class ToolWindow(Gtk.Window):
    def __init__(self):
        super().__init__(title="Smart CPU Optimizer")  # Changed window title
    ...
    def main(self):
        ...
        # Left system stats box
        self.systemstats = SystemStatsLabel()
        self.hbox.pack_start(self.systemstats, False, False, 0)
        self.add(self.hbox)

        # Right-side panel
        self.vbox_right = Gtk.Box(orientation=Gtk.Orientation.VERTICAL, spacing=48)
    ...
    def snap(self):
        ...
        label = Gtk.Label(label="GUI unavailable due to Snap confinement.\nUse the installer or visit GitHub for info.")
        label.set_justify(Gtk.Justification.CENTER)
        button = Gtk.LinkButton.new_with_label(uri=GITHUB, label="Visit GitHub")
    ...
    def handle_update(self):
        ...
        if updater.stderr == PKEXEC_ERROR:
            error = Gtk.MessageDialog(self, 0, Gtk.MessageType.ERROR, Gtk.ButtonsType.OK, "Update failed")
            error.format_secondary_text("Authorization error — please try again with permissions.")
            error.run()
            error.destroy()
            return

        success = Gtk.MessageDialog(self, 0, Gtk.MessageType.INFO, Gtk.ButtonsType.OK, "Updated successfully!")
        success.format_secondary_text("App will close now. Please restart it.")
        success.run()
        success.destroy()
        exit(0)
