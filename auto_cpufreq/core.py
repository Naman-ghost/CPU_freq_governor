...
from auto_cpufreq.profiles import detect_profile, get_profile_settings
from auto_cpufreq.battery_helper import set_charge_threshold, get_battery_health
...
def set_powersave():
    ...
    # --- Set governor ---
    def set_governor(conf):
        gov = conf["battery"].get("governor", AVAILABLE_GOVERNORS_SORTED[-1])
    ...
    # --- Apply EPP (Energy Performance Preference) ---
    def apply_epp(conf, gov):
        epp_supported = Path("/sys/devices/system/cpu/cpu0/cpufreq/energy_performance_preference").exists()
        if not epp_supported:
    ...
    # --- Decide turbo ---
    def decide_turbo(conf):
        cpuload, load1m = get_load()
        auto = conf["battery"].get("turbo", "auto")
...
def set_balanced():
    """
    Applies the balanced profile settings
    """
    from auto_cpufreq.utils import run_cmd  # usually used for shell commands

    # Example: balanced governor
    run_cmd("cpupower frequency-set -g powersave")  # or performance depending on AC/battery
    run_cmd("echo 'auto' | sudo tee /sys/devices/system/cpu/cpu*/cpufreq/boost")

def set_performance():
    conf = config.get_config()
    gov = conf.get("charger", {}).get("governor", AVAILABLE_GOVERNORS_SORTED[0])
    ...
    if not epp_path.exists():
        ...
        def apply_epp(pstate_path):
            active = Path(pstate_path).exists() and open(pstate_path, 'r').read().strip() == "active"
            epp = conf.get("charger", {}).get("energy_performance_preference")

            if active and epp and gov == "performance" and epp != "performance":
        ...
        if Path("/sys/devices/system/cpu/intel_pstate").exists():
            dynboost_path = "/sys/devices/system/cpu/intel_pstate/hwp_dynamic_boost"
            dynboost_enabled = Path(dynboost_path).exists() and bool(int(open(dynboost_path).read().strip()))
            if dynboost_enabled:
                print('Not setting EPP (dynamic boosting is enabled)')
            else:
                apply_epp("/sys/devices/system/cpu/intel_pstate/status")
        elif Path("/sys/devices/system/cpu/amd_pstate").exists():
            apply_epp("/sys/devices/system/cpu/amd_pstate/status")
    ...
    cpuload, load1m = get_load()
    auto = get_turbo_override() if get_turbo_override() != "auto" else conf.get("charger", {}).get("turbo", "auto")
...
def apply_profile(profile: str):
    """
    Apply CPU profile: 'powersave', 'performance', or 'balanced'.
    """
